...
object BreakingActorEncapsulation {
      ...
      Behaviors.receiveMessage {
        ...
        case WithDraw(cardId, amount) =>
          ...
          if (oldBalance < amount) {
            ...
            context.log.info(s"WithDraw $amount via card $cardId, balance on card: ${oldBalance - amount}")
            accountBalances += cardId -> (oldBalance - amount)
            Behaviors.same
          }
  ...
  def main(args: Array[String]): Unit = {
    val userGuardian: Behavior[Unit] = Behaviors.setup { context =>
      val bankAccount = context.spawn(NativeBankAccount(), "NativeBankAccount")

      bankAccount ! CreateCreditCard("gold")
      bankAccount ! CreateCreditCard("premium")
      bankAccount ! Deposit("gold", 1000)
      bankAccount ! WithDraw("gold", 100)
      bankAccount ! CheckCardStatuses

      Behaviors.empty
    }

    val system = ActorSystem(userGuardian, "DemoNativeAccount")
    Thread.sleep(1000)
    system.terminate()
  }
}
