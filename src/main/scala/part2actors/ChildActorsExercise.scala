...
object ChildActorsExercise {
   ...
   *    WCM: word count master
   *    requester ------ (computational task) ------> WCM (computational task) -------> one child of type WCM
   *    requester <----- (computational res) <------ WCM (computational res) <------- one child of type WCM
   *
    ...
    def apply(): Behavior[MasterProtocol] = Behaviors.receive { (context, message) =>
      message match {
        case Initialize(nChildren) =>
          ...
          val childRefs = for {
            i <- 1 to nChildren
          } yield context.spawn(WorldCounterWorker(context.self), s"worker$i")
  ...
  def testWordCounter(): Unit = {
    val userGuardian: Behavior[Unit] = Behaviors.setup { context =>
      ...
      val wcm = context.spawn(WorldCounterMaster(), "master")
