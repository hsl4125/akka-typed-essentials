...
object AskDemo {
  ...
  def askSimple(): Unit = {
    ...
    val reply: Future[WorkProtocol] = system.ask { ref =>
      //                                           ^ temporary actor
      ComputationalTask("Trying the ask pattern, seems convoluted", ref)
      // ^ message that gets sent to the worker == the user guardian
    }
  ...
  def askFromWithinAnotherActor(): Unit = {
    val userGuardian = Behaviors.setup[WorkProtocol] { context =>
      ...
      context.ask(worker, ref => ComputationalTask("This ask pattern seems quite complicated", ref)) {
        // Try[WorkProtocol] => WorkProtocol message that be sent TO ME later
        case Success(ComputationalResult(count)) => ExtendedComputationalResult(count, "This is pretty damn hard")
        case Failure(ex) => ExtendedComputationalResult(-1, "Computation failed: $ex")
      }
