...
class InterceptingLogsSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "A checkout flow" should {
    "correctly dispatch an order for valid credit card" in {
      LoggingTestKit.info("Order")  // filter log message of level INFO which the string "Order"
        .withMessageRegex(s"Order [0-9]+ for $item has been dispatched.")
        .withOccurrences(1)
        .expect {
          // scenario under test
...
object InterceptingLogsSpec {
  ...
  case class AuthorizeCar(creditCard: String, replyTo: ActorRef[PaymentProtocol]) extends PaymentProtocol
    ...
    def apply(): Behavior[PaymentProtocol] = Behaviors.setup { context =>
      ...
      def awaitingCheckout(): Behavior[PaymentProtocol] = Behaviors.receiveMessage {
        case Checkout(item, card) =>
          ...
          paymentManager ! AuthorizeCar(card, context.self)
          pendingPayment(item)
      ...
      def pendingDispatch(item: String): Behavior[PaymentProtocol] = Behaviors.receiveMessage {
        case OrderConfirmed =>
          context.log.info("Dispatch confirmed")
          awaitingCheckout()
      }
  ...
  object PaymentManager {
    def apply(): Behavior[PaymentProtocol] = Behaviors.receiveMessage {
      case AuthorizeCar(card, replyTo) =>
    ...
    def active(orderId: Int): Behavior[PaymentProtocol] = Behaviors.receive { (context, message) =>
      message match {
        case DispatchOrder(item, replyTo) =>
          context.log.info(s"Order $orderId for $item has been dispatched.")
